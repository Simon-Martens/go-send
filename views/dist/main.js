...
var localeLoaders = {
  ...
  de: () => import("./chunks/de-Y6RNQEHR.js"),
...
// src/ui/file-edit.mjs
var FileEditView = class extends HTMLElement {
  constructor() {
    super();
    this.ownedFile = null;
    this._templateMounted = false;
  ...
  setFile(ownedFile) {
    ...
    this._isMultiFile = ownedFile.manifest && ownedFile.manifest.files && ownedFile.manifest.files.length > 1;
    ...
    this._newTimeLimit = Math.max(0, Math.floor(remainingMs / 1e3));
  ...
  _renderFileInfo() {
    ...
    if (this.elements.fileIcon && this.ownedFile.name) {
      ...
      if (fileName.endsWith(".zip")) {
        this.elements.fileIcon.className = "ri-folder-6-line h-8 w-8 flex-shrink-0 text-primary mr-3 text-3xl leading-8";
      } else {
        this.elements.fileIcon.className = "ri-file-ai-line h-8 w-8 flex-shrink-0 text-primary mr-3 text-3xl leading-8";
      }
    }
  ...
  _renderExpiryControls() {
    ...
    if (this.elements.downloadLimit) {
      ...
      if (downloadCounts.includes(this._originalDlimit) || downloadCounts.length === 0) {
        this.elements.downloadLimit.value = String(this._originalDlimit);
      } else {
        const closest = downloadCounts.reduce(
          (prev, curr) => Math.abs(curr - this._originalDlimit) < Math.abs(prev - this._originalDlimit) ? curr : prev
        );
    ...
    if (this.elements.timeLimit) {
      ...
      const minTimeSeconds = Math.ceil(timeSinceCreation / 1e3);
      ...
      if (expireTimes.includes(this._newTimeLimit)) {
        ...
        const closest = expireTimes.reduce(
          (prev, curr) => Math.abs(curr - this._newTimeLimit) < Math.abs(prev - this._newTimeLimit) ? curr : prev
        );
  ...
  handleDownloadLimitChange(event) {
    ...
    console.log("[FileEditView] Save button disabled?", (_a = this.elements.saveButton) == null ? void 0 : _a.disabled);
  }
  ...
  async handleSave(event) {
    ...
    console.log("[FileEditView] Changes:", {
      ...
      expiryChanged: Math.abs(Date.now() + this._newTimeLimit * 1e3 - this._originalExpiresAt) > 1e3,
    ...
    try {
      ...
      const newExpiresAt = Date.now() + this._newTimeLimit * 1e3;
      if (Math.abs(newExpiresAt - this._originalExpiresAt) > 1e3) {
      ...
      storage_default.writeFile(this.ownedFile);
  ...
  _showNameError(errorKey) {
    const errorEl = this._isMultiFile ? this.elements.archiveNameError : this.elements.nameError;
    const inputEl = this._isMultiFile ? this.elements.archiveNameInput : this.elements.fileNameInput;
    if (!errorEl || !inputEl) {
      return;
    }
  ...
  _formatDate(timestamp) {
    ...
    const locale2 = document.documentElement.lang || navigator.language || "en";
...
async function initRequestInvitationRoute(app) {
  console.log("[Route] Initializing request invitation page...");
  await Promise.all([
    import("./chunks/request-invitation-layout-TPEDIED2.js"),
    app.controller.ready
  ]);
