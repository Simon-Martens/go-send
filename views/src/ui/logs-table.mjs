...
/**
 * Generic LogsTable component for displaying file transfer logs
 * Can be used standalone or within other components
 * Supports optional fileId parameter to filter logs for a specific file
 */
class LogsTable extends HTMLElement {
  constructor() {
    super();
    ...
    // State
    this._fileId = null; // Optional file ID filter
    this._currentPage = 1;
    this._totalCount = 0;
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    if (name === "fileId" && oldValue !== newValue) {
      ...
      // Reset to page 1 when fileId changes
      this._currentPage = 1;
      if (this._tableBody) {
        this._fetchLogs(1);
      }
    ...
    if (name === "hideHeader" && oldValue !== newValue) {
      // Show/hide header based on attribute
  ...
  connectedCallback() {
    // Extract fileId attribute if set
    ...
    // Hide header if hideHeader attribute is set
  ...
  async _fetchLogs(page) {
    ...
    try {
      // Call fetchLogs with optional fileId parameter
  ...
  _createLogRow(log) {
    ...
    // Auth token icon (ticket icon for files uploaded via auth token)
  ...
  _updatePaginationUI() {
    // Update log count display with localized format
    if (this._countText) {
      ...
      if (this._currentLogCount > 0) {
        // Use translation with parameters for localized format
